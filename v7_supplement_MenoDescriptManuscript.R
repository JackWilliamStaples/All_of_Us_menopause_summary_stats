...
# This query represents dataset "v7_supplement_srWGS" for domain "person" and was generated for All of Us Controlled Tier Dataset v7
dataset_28924392_person_sql <- paste("
    SELECT
        person.person_id 
    FROM
        `person` person   
...
person_28924392_path <- file.path(
  Sys.getenv("WORKSPACE_BUCKET"),
  "bq_exports",
  Sys.getenv("OWNER_EMAIL"),
  strftime(lubridate::now(), "%Y%m%d"),  # Comment out this line if you want the export to always overwrite.
  "person_28924392",
  "person_28924392_*.csv")
message(str_glue('The data will be written to {person_28924392_path}. Use this path when reading ',
...
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_28924392_person_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  person_28924392_path,
  destination_format = "CSV")

# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {person_28924392_path}` to copy these files
#       to the Jupyter disk.
...
dataset_28924392_person_df <- read_bq_export_from_workspace_bucket(person_28924392_path)

dim(dataset_28924392_person_df)

head(dataset_28924392_person_df, 5)

print("unique person IDs of all individuals with sr-WGS data ")
Has_srWGS_all_personIDs <- 
  dataset_28924392_person_df %>%
...
# This query represents dataset "v7_supplement_srWGS_females" for domain "person" and was generated for All of Us Controlled Tier Dataset v7
dataset_81811970_person_sql <- paste("
    SELECT
        person.person_id,
...
#       But data exported on a different days will write to a new location so that historical
#       copies can be kept as the dataset definition is changed.
person_81811970_path <- file.path(
  Sys.getenv("WORKSPACE_BUCKET"),
  "bq_exports",
  Sys.getenv("OWNER_EMAIL"),
  strftime(lubridate::now(), "%Y%m%d"),  # Comment out this line if you want the export to always overwrite.
  "person_81811970",
  "person_81811970_*.csv")
message(str_glue('The data will be written to {person_81811970_path}. Use this path when reading ',
                 'the data into your notebooks in the future.'))
...
#       just read data from the CSVs in Cloud Storage.
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_81811970_person_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  person_81811970_path,
  destination_format = "CSV")


# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {person_81811970_path}` to copy these files
#       to the Jupyter disk.
read_bq_export_from_workspace_bucket <- function(export_path) {
...
Has_srWGS_and_Female_sex_personIDs <- read_bq_export_from_workspace_bucket(person_81811970_path)
...
# This query represents dataset "v7_supplement_survey" for domain "survey" and was generated for All of Us Controlled Tier Dataset v7
dataset_22344914_survey_sql <- paste("
    SELECT
        answer.person_id  
    FROM
        `ds_survey` answer    
                    ...
                    WHERE
                        ...
                        JOIN
                            ...
                            FROM
                                `cb_criteria` cr       
                            WHERE
                                concept_id IN (1585855, 1740639, 1586134, 1333342, 1585710, 40192389, 1741006, 43528895)       
...
#       But data exported on a different days will write to a new location so that historical
#       copies can be kept as the dataset definition is changed.
survey_22344914_path <- file.path(
  Sys.getenv("WORKSPACE_BUCKET"),
  "bq_exports",
  Sys.getenv("OWNER_EMAIL"),
  strftime(lubridate::now(), "%Y%m%d"),  # Comment out this line if you want the export to always overwrite.
  "survey_22344914",
  "survey_22344914_*.csv")
message(str_glue('The data will be written to {survey_22344914_path}. Use this path when reading ',
                 'the data into your notebooks in the future.'))
...
#       just read data from the CSVs in Cloud Storage.
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_22344914_survey_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  survey_22344914_path,
  destination_format = "CSV")


# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {survey_22344914_path}` to copy these files
#       to the Jupyter disk.
read_bq_export_from_workspace_bucket <- function(export_path) {
...
Has_survey_all_AoURP_personIDs <- read_bq_export_from_workspace_bucket(survey_22344914_path)
...
# This query represents dataset "v7_supplement_EHR" for domain "person" and was generated for All of Us Controlled Tier Dataset v7
dataset_84964837_person_sql <- paste("
    SELECT
        person.person_id 
    FROM
        `person` person   
...
#       But data exported on a different days will write to a new location so that historical
#       copies can be kept as the dataset definition is changed.
person_84964837_path <- file.path(
  Sys.getenv("WORKSPACE_BUCKET"),
  "bq_exports",
  Sys.getenv("OWNER_EMAIL"),
  strftime(lubridate::now(), "%Y%m%d"),  # Comment out this line if you want the export to always overwrite.
  "person_84964837",
  "person_84964837_*.csv")
message(str_glue('The data will be written to {person_84964837_path}. Use this path when reading ',
                 'the data into your notebooks in the future.'))
...
#       just read data from the CSVs in Cloud Storage.
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_84964837_person_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  person_84964837_path,
  destination_format = "CSV")


# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {person_84964837_path}` to copy these files
#       to the Jupyter disk.
read_bq_export_from_workspace_bucket <- function(export_path) {
  col_types <- NULL
  bind_rows(
...
dataset_84964837_person_df <- read_bq_export_from_workspace_bucket(person_84964837_path)

dim(dataset_84964837_person_df)

head(dataset_84964837_person_df, 5)


print("unique person IDs of all individuals with EHR data ")
Has_EHR_all_AoURP_personIDs <- 
  dataset_84964837_person_df %>%
...
# This query represents dataset "v7_supplement_EHR_MenopauseDiagnosticCode" for domain "person" and was generated for All of Us Controlled Tier Dataset v7
dataset_32906329_person_sql <- paste("
    SELECT
        person.person_id 
    FROM
        `person` person   
...
#       But data exported on a different days will write to a new location so that historical
#       copies can be kept as the dataset definition is changed.
person_32906329_path <- file.path(
  Sys.getenv("WORKSPACE_BUCKET"),
  "bq_exports",
  Sys.getenv("OWNER_EMAIL"),
  strftime(lubridate::now(), "%Y%m%d"),  # Comment out this line if you want the export to always overwrite.
  "person_32906329",
  "person_32906329_*.csv")
message(str_glue('The data will be written to {person_32906329_path}. Use this path when reading ',
                 'the data into your notebooks in the future.'))
...
#       just read data from the CSVs in Cloud Storage.
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_32906329_person_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  person_32906329_path,
  destination_format = "CSV")


# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {person_32906329_path}` to copy these files
#       to the Jupyter disk.
read_bq_export_from_workspace_bucket <- function(export_path) {
  col_types <- NULL
  bind_rows(
...
dataset_32906329_person_df <- read_bq_export_from_workspace_bucket(person_32906329_path)

dim(dataset_32906329_person_df)

head(dataset_32906329_person_df, 5)


Has_EHR_MenopauseDiagnosticCode_all_personIDs <- 
  dataset_32906329_person_df %>%
...
# This query represents dataset "v7_supplement_SurveyMenstrualStoppedYes" for domain "person" and was generated for All of Us Controlled Tier Dataset v7
dataset_94459835_person_sql <- paste("
    SELECT
        person.person_id 
    FROM
        `person` person   
...
#       But data exported on a different days will write to a new location so that historical
#       copies can be kept as the dataset definition is changed.
person_94459835_path <- file.path(
  Sys.getenv("WORKSPACE_BUCKET"),
  "bq_exports",
  Sys.getenv("OWNER_EMAIL"),
  strftime(lubridate::now(), "%Y%m%d"),  # Comment out this line if you want the export to always overwrite.
  "person_94459835",
  "person_94459835_*.csv")
message(str_glue('The data will be written to {person_94459835_path}. Use this path when reading ',
                 'the data into your notebooks in the future.'))
...
#       just read data from the CSVs in Cloud Storage.
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_94459835_person_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  person_94459835_path,
  destination_format = "CSV")


# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {person_94459835_path}` to copy these files
#       to the Jupyter disk.
read_bq_export_from_workspace_bucket <- function(export_path) {
  col_types <- NULL
  bind_rows(
...
dataset_94459835_person_df <- read_bq_export_from_workspace_bucket(person_94459835_path)

dim(dataset_94459835_person_df)

head(dataset_94459835_person_df, 5)
...
# This query represents dataset "v7_supplement_SurveyMenstrualStopped_AnyAnswer" for domain "person" and was generated for All of Us Controlled Tier Dataset v7
dataset_98831045_person_sql <- paste("
    SELECT
        person.person_id 
    FROM
        `person` person   
        ...
        WHERE
            ...
            FROM
                ...
                WHERE
                    ...
                    OR  concept_id IN (1585784) 
                    AND is_standard = 0  
                    AND  value_source_concept_id IN (903096))) criteria ) )", sep="")

# Formulate a Cloud Storage destination path for the data exported from BigQuery.
# NOTE: By default data exported multiple times on the same day will overwrite older copies.
#       But data exported on a different days will write to a new location so that historical
#       copies can be kept as the dataset definition is changed.
person_98831045_path <- file.path(
  Sys.getenv("WORKSPACE_BUCKET"),
  "bq_exports",
  Sys.getenv("OWNER_EMAIL"),
  strftime(lubridate::now(), "%Y%m%d"),  # Comment out this line if you want the export to always overwrite.
  "person_98831045",
  "person_98831045_*.csv")
message(str_glue('The data will be written to {person_98831045_path}. Use this path when reading ',
                 'the data into your notebooks in the future.'))
...
#       just read data from the CSVs in Cloud Storage.
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_98831045_person_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  person_98831045_path,
  destination_format = "CSV")


# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {person_98831045_path}` to copy these files
#       to the Jupyter disk.
read_bq_export_from_workspace_bucket <- function(export_path) {
  col_types <- NULL
  bind_rows(
...
dataset_98831045_person_df <- read_bq_export_from_workspace_bucket(person_98831045_path)

dim(dataset_98831045_person_df)

head(dataset_98831045_person_df, 5)
...
if(!dir.exists(paths = "./v7_supplement_MenoDescriptManuscript/"))
{
  dir.create(path = "./v7_supplement_MenoDescriptManuscript/")
}
...
Menopause_IntersectionVenn_Table %>%
  write_tsv(
    x = .,
    file = "./v7_supplement_MenoDescriptManuscript/Menopause_IntersectionVenn_Table.tsv"
  )
...
# This query represents dataset "v7_supplement_Granular_Menopause_EHR_Survey" for domain "condition" and was generated for All of Us Controlled Tier Dataset v7
dataset_96377517_condition_sql <- paste("
    SELECT
            ...
            (
                ...
                JOIN
                    ...
                    FROM
                        `cb_criteria` cr       
                    WHERE
                        concept_id IN (198715, 4028935, 4128329, 4129547, 4279913, 4322635)       
                        AND full_text LIKE '%_rank1]%'      ) a 
                        ON (c.path LIKE CONCAT('%.', a.id, '.%') 
                        OR c.path LIKE CONCAT('%.', a.id) 
                        OR c.path LIKE CONCAT(a.id, '.%') 
...
#       But data exported on a different days will write to a new location so that historical
#       copies can be kept as the dataset definition is changed.
condition_96377517_path <- file.path(
  Sys.getenv("WORKSPACE_BUCKET"),
  "bq_exports",
  Sys.getenv("OWNER_EMAIL"),
  strftime(lubridate::now(), "%Y%m%d"),  # Comment out this line if you want the export to always overwrite.
  "condition_96377517",
  "condition_96377517_*.csv")
message(str_glue('The data will be written to {condition_96377517_path}. Use this path when reading ',
                 'the data into your notebooks in the future.'))
...
#       just read data from the CSVs in Cloud Storage.
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_96377517_condition_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  condition_96377517_path,
  destination_format = "CSV")


# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {condition_96377517_path}` to copy these files
#       to the Jupyter disk.
read_bq_export_from_workspace_bucket <- function(export_path) {
...
Granular_Menopause_EHR_condition_data <- read_bq_export_from_workspace_bucket(condition_96377517_path)
...
Granular_Menopause_EHR_condition_data_clean_final_oneHot$person_id %>% length()
Granular_Menopause_EHR_condition_data_clean_final_oneHot$person_id %>% unique() %>% length()
Granular_Menopause_EHR_condition_data_clean_final_oneHot$conditionEHR_Menopause_present_289903006_SNOMED %>% sum()

library(tidyverse)
library(bigrquery)

# This query represents dataset "v7_supplement_Granular_Menopause_EHR_Survey" for domain "survey" and was generated for All of Us Controlled Tier Dataset v7
dataset_96377517_survey_sql <- paste("
    SELECT
        answer.person_id,
...
#       But data exported on a different days will write to a new location so that historical
#       copies can be kept as the dataset definition is changed.
survey_96377517_path <- file.path(
  Sys.getenv("WORKSPACE_BUCKET"),
  "bq_exports",
  Sys.getenv("OWNER_EMAIL"),
  strftime(lubridate::now(), "%Y%m%d"),  # Comment out this line if you want the export to always overwrite.
  "survey_96377517",
  "survey_96377517_*.csv")
message(str_glue('The data will be written to {survey_96377517_path}. Use this path when reading ',
                 'the data into your notebooks in the future.'))
...
#       just read data from the CSVs in Cloud Storage.
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_96377517_survey_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  survey_96377517_path,
  destination_format = "CSV")


# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {survey_96377517_path}` to copy these files
#       to the Jupyter disk.
read_bq_export_from_workspace_bucket <- function(export_path) {
...
Granular_Menopause_Survey_Response <- read_bq_export_from_workspace_bucket(survey_96377517_path)
...
# # Demographic data (full AoURP cohort)
...
# This query represents dataset "v7_supplement_demographic" for domain "survey" and was generated for All of Us Controlled Tier Dataset v7
dataset_58375540_survey_sql <- paste("
    SELECT
        answer.person_id,
        answer.survey_datetime,
        answer.survey,
        ...
        AND (
            ...
            WHERE
                ...
                FROM
                    `cb_search_person` p 
                WHERE
                    DATE_DIFF(CURRENT_DATE, dob, YEAR) - IF(EXTRACT(MONTH FROM dob)*100 + EXTRACT(DAY FROM dob) > EXTRACT(MONTH FROM CURRENT_DATE)*100 + EXTRACT(DAY FROM CURRENT_DATE), 1, 0) BETWEEN 18 AND 125 
                ...
                FROM
                    `person` p 
                WHERE
                    gender_concept_id IN (45878463, 45880669, 903096, 2000000002, 1585841, 1177221, 1585842, 1585843, 0) 
                UNION
                DISTINCT SELECT
                    person_id 
                FROM
                ...
                FROM
                    `person` p 
                WHERE
                    sex_at_birth_concept_id IN (45878463, 45880669, 0, 903096, 1177221, 4124462, 46273637) 
                UNION
                DISTINCT SELECT
                    person_id 
                FROM
...
#       But data exported on a different days will write to a new location so that historical
#       copies can be kept as the dataset definition is changed.
survey_58375540_path <- file.path(
  Sys.getenv("WORKSPACE_BUCKET"),
  "bq_exports",
  Sys.getenv("OWNER_EMAIL"),
  strftime(lubridate::now(), "%Y%m%d"),  # Comment out this line if you want the export to always overwrite.
  "survey_58375540",
  "survey_58375540_*.csv")
message(str_glue('The data will be written to {survey_58375540_path}. Use this path when reading ',
                 'the data into your notebooks in the future.'))
...
#       just read data from the CSVs in Cloud Storage.
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_58375540_survey_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  survey_58375540_path,
  destination_format = "CSV")


# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {survey_58375540_path}` to copy these files
#       to the Jupyter disk.
read_bq_export_from_workspace_bucket <- function(export_path) {
  col_types <- cols(survey = col_character(), question = col_character(), answer = col_character(), survey_version_name = col_character())
  bind_rows(
...
dataset_58375540_survey_df <- read_bq_export_from_workspace_bucket(survey_58375540_path)

dim(dataset_58375540_survey_df)

head(dataset_58375540_survey_df, 5)

library(tidyverse)
library(bigrquery)

# This query represents dataset "v7_supplement_demographic" for domain "person" and was generated for All of Us Controlled Tier Dataset v7
dataset_58375540_person_sql <- paste("
    SELECT
        person.person_id,
...
#       But data exported on a different days will write to a new location so that historical
#       copies can be kept as the dataset definition is changed.
person_58375540_path <- file.path(
  Sys.getenv("WORKSPACE_BUCKET"),
  "bq_exports",
  Sys.getenv("OWNER_EMAIL"),
  strftime(lubridate::now(), "%Y%m%d"),  # Comment out this line if you want the export to always overwrite.
  "person_58375540",
  "person_58375540_*.csv")
message(str_glue('The data will be written to {person_58375540_path}. Use this path when reading ',
                 'the data into your notebooks in the future.'))
...
#       just read data from the CSVs in Cloud Storage.
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_58375540_person_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  person_58375540_path,
  destination_format = "CSV")


# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {person_58375540_path}` to copy these files
#       to the Jupyter disk.
read_bq_export_from_workspace_bucket <- function(export_path) {
...
dataset_58375540_person_df <- read_bq_export_from_workspace_bucket(person_58375540_path)

dim(dataset_58375540_person_df)

head(dataset_58375540_person_df, 5)
...
# This query represents dataset "v7_supplement_insurance_data_basic_survey_for_menopause" for domain "survey" and was generated for All of Us Controlled Tier Dataset v7
dataset_21634649_survey_sql <- paste("
    SELECT
        answer.person_id,
        answer.survey_datetime,
        answer.survey,
                    ...
                    WHERE
                        ...
                        JOIN
                            ...
                            FROM
                                `cb_criteria` cr       
                            WHERE
                                concept_id IN (1585386, 1585389, 43528428)       
                                AND full_text LIKE '%_rank1]%'      ) a 
                                ON (c.path LIKE CONCAT('%.', a.id, '.%') 
                                OR c.path LIKE CONCAT('%.', a.id) 
                                OR c.path LIKE CONCAT(a.id, '.%') 
...
#       But data exported on a different days will write to a new location so that historical
#       copies can be kept as the dataset definition is changed.
survey_21634649_path <- file.path(
  Sys.getenv("WORKSPACE_BUCKET"),
  "bq_exports",
  Sys.getenv("OWNER_EMAIL"),
  strftime(lubridate::now(), "%Y%m%d"),  # Comment out this line if you want the export to always overwrite.
  "survey_21634649",
  "survey_21634649_*.csv")
message(str_glue('The data will be written to {survey_21634649_path}. Use this path when reading ',
                 'the data into your notebooks in the future.'))
...
#       just read data from the CSVs in Cloud Storage.
bq_table_save(
  bq_dataset_query(Sys.getenv("WORKSPACE_CDR"), dataset_21634649_survey_sql, billing = Sys.getenv("GOOGLE_PROJECT")),
  survey_21634649_path,
  destination_format = "CSV")


# Read the data directly from Cloud Storage into memory.
# NOTE: Alternatively you can `gsutil -m cp {survey_21634649_path}` to copy these files
#       to the Jupyter disk.
read_bq_export_from_workspace_bucket <- function(export_path) {
  col_types <- cols(survey = col_character(), question = col_character(), answer = col_character(), survey_version_name = col_character())
  bind_rows(
...
insurance_basic_survey_df <- read_bq_export_from_workspace_bucket(survey_21634649_path)
...
insurance_basic_survey_df_clean <-
  ...
  # keep earliest insurance questions response per person
  group_by(
    .data = .,
    person_id
  ) %>%
...
demographic_income_education_survey_clean <-
  ...
  # filter to earliest question response per person
  group_by(
    .data = .,
    person_id
  ) %>%
...
# write_tsv(
#     x = .,
#     file = "./v7_supplement_MenoDescriptManuscript/ehr_consented_pids_df_V7_freeze09182025.tsv"
#     )
...
# write_tsv(
#     x = .,
#     file = "./v7_supplement_MenoDescriptManuscript/MenopauseMasterEHRsurveyWGSsociodemographicsFile_V7_freeze09182025.tsv"
#     )
...
EHR_menopause_code_repeats_table %>%
  write_tsv(
    x = .,
    file = "./v7_supplement_MenoDescriptManuscript/EHR_menopause_code_repeats_table.tsv"
  )
...
IntersectSampleSize_menopause_EHR_survey_combinations %>%
  ...
  write_tsv(
    x = .,
    file = "./v7_supplement_MenoDescriptManuscript/IntersectSampleSize_menopause_EHR_survey_combinations.tsv"
  )
...
IntersectSampleSize_menopause_EHR_survey_combinations %>%
  ...
  write_tsv(
    x = .,
    file = "./v7_supplement_MenoDescriptManuscript/Table3_age_above60_menopause_EHR_survey_combinations.tsv"
  )
...
Menopause_demographic_stat_summary_srWGS_EHR_Survey_granular_EHR_and_survey %>%
  write_tsv(
    x = .,
    file = "./v7_supplement_MenoDescriptManuscript/Menopause_demographic_stat_summary_srWGS_EHR_Survey_granular_EHR_and_survey_ageDateAdjust_femaleSexFilter.tsv"
  )
...
Menopause_descriptor_sample_sizes_noStrata %>%
  write_tsv(
    x = .,
    file = "./v7_supplement_MenoDescriptManuscript/Table1_Menopause_descriptor_sample_sizes_noStrata.tsv"
  )
...
Menopause_demographic_stat_summary_srWGS_EHR_Survey_granular_EHR_and_survey_forSummary$Descriptor %>%
  unique()
...
Demographic_summary_stats_table %>%
  write_tsv(
    x = .,
    file = "./v7_supplement_MenoDescriptManuscript/Table2_Demographic_summary_stats_table.tsv"
  )
    ...
    FUN = function(currentDescriptorDataframe)
    {
      ...
      InputDataForPlot %>%
        write_tsv(
          x = .,
          file = paste0(
            "./v7_supplement_MenoDescriptManuscript/",
      ...
      ggsave(
        ...
        units = "px",
        path = "./v7_supplement_MenoDescriptManuscript/",
        filename = paste0(
          "SampleSize_Descriptor_allStrataAllgroups_",
          currentDescriptor,
          ".png"
